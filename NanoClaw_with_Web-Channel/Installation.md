...
- Sessions from other devices appear immediately via SSE `sessions_changed` event; 60 s polling as fallback
- Unread message indicators (blue dot) per session, persisted in `localStorage`
- Sessions can be deleted (removes all messages from DB and `localStorage`; all other clients are notified immediately via `sessions_changed`)
- Sidebar width is adjustable by dragging the grip handle; width is persisted in `localStorage`
...
- Individual messages can be deleted (trash icon; removes from DOM, cache, and DB; all other clients are notified via `delete_message` SSE event)
- User messages have a copy button (top-right corner of the bubble)
...
| `user_message` | `{"text":"…","id":"…"}` | User message sent from another device (shown immediately) |
| `delete_message` | `{"id":"…"}` | Remove a message from the UI on all clients |
| `sessions_changed` | `{"deleted":"…"}` / `{"added":"…"}` / `{"renamed":"…"}` | Trigger session list refresh on all clients |
| `ping` | *(empty)* | Heartbeat every 20 s — keeps SSE alive on Android; used for stale-connection detection |

On connect, the server immediately sends the current `typing`, `status`, and `cwd` state so reconnects are seamless. The client reconnects automatically if no `ping` is received within 35 s (detects frozen Android SSE connections).
...
NanoClaw can be installed as a PWA on mobile home screens and desktops:

- On Android/Chrome: browser menu → *Add to Home Screen*
- On iOS/Safari: Share → *Add to Home Screen*
- On Desktop Chrome/Edge: install icon in the address bar

The app runs in standalone mode (no browser chrome) and always loads the current version directly from the server — no service worker, no offline cache.
...
| `GET` | `/manifest.json` | PWA web app manifest (enables "Add to Home Screen") |
...
| `POST` | `/delete-message` | Delete a single message `{sid, id}` — broadcasts `delete_message` to all clients |
...
GET /sessions  ←── poll 60s ─  getAllChats() + sessionCwds   chats table
...
POST /delete-session  ──────→  deleteChat() + broadcastAll('sessions_changed')   chats + messages
POST /delete-message  ──────→  deleteMessage(id) + broadcastAll('delete_message')   messages table
POST /clear-history   ──────→  clearChatMessages()           messages table
...
| `src/channels/web.ts` | Complete HTTP server with embedded browser chat UI (HTML/CSS/JS inline); multi-session sidebar with drag-to-reorder and resizable width; real-time SSE with `message`, `typing`, `status`, `cwd`, `user_message`, `delete_message`, `sessions_changed`, and `ping` events; PWA manifest; message delete with cross-device broadcast; copy buttons on bot and user messages; Android SSE heartbeat and stale-connection recovery |
...
| `container/skills/agent-browser/SKILL.md` | Built-in skill: web browsing via `agent-browser` CLI |
| `container/skills/capabilities/SKILL.md` | Built-in skill: describes the agent's capabilities to callers |
| `container/skills/slack-formatting/SKILL.md` | Built-in skill: Slack-compatible message formatting |
| `container/skills/status/SKILL.md` | Built-in skill: tool-use status reporting |
