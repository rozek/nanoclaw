...
function readClaudeOAuthToken() {
    if (process.platform === 'darwin') {
        for (const svc of ['Claude Code-credentials', 'Claude Code']) {
            try {
                const json = execSync(`security find-generic-password -s "${svc}" -w 2>/dev/null`, { encoding: 'utf-8' }).trim();
                if (!json)
                    continue;
...
async function cli() {
    ...
    if (sandboxRaw) {
        ...
        sandboxType = sandboxRaw;
    }
    else {
        const detected = detectSandbox();
        if (!detected) {
            die('No container sandbox found.\n' +
    ...
    if (sandboxType === 'docker') {
        ...
        // apple
        if (!checkBinary('container --version')) {
            die('Apple Container Runtime not available.\n' +
    ...
    if (host)
        process.env.NANOCLAW_HOST = host;
    if (portStr)
        process.env.NANOCLAW_PORT = portStr;
    if (key)
        process.env.ANTHROPIC_API_KEY = key;
    if (token)
        process.env.NANOCLAW_TOKEN = token;
