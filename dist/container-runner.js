...
import { CONTAINER_IMAGE, CONTAINER_MAX_OUTPUT_SIZE, CONTAINER_TIMEOUT, DATA_DIR, GROUPS_DIR, IDLE_TIMEOUT, ONECLI_URL, TIMEZONE, } from './config.js';
...
export async function runContainerAgent(group, input, onProcess, onOutput, onStatus) {
                    ...
                    if (statusStart !== -1 &&
                        ...
                        if (endIdx !== -1) {
                            const jsonStr = parseBuffer
                                ...
                                .trim();
                            parseBuffer = parseBuffer.slice(endIdx + STATUS_END_MARKER.length);
                            found = true;
                            if (onStatus) {
                                try {
                                    ...
                                    logger.debug({ group: group.name, error: err }, 'Failed to parse status marker (non-critical)');
                                }
                            }
                            continue;
                        }
