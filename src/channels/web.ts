  ...
  if (raw !== undefined) {
    try {
      if (decodeURIComponent(raw) === TOKEN) {
        ...
        res.setHeader(
          'Set-Cookie',
          `nanoclaw_token=${TOKEN}; HttpOnly; SameSite=Strict; Path=/${isHttps ? '; Secure' : ''}`,
        );
        return true;
      }
    } catch {
...
function sanitizeSid(raw: unknown): string {
      ...
      if (req.method === 'POST' && req.url === '/session-order') {
        collectBody(req, res, (body) => {
          try {
            ...
            setWebSessionOrder(
              order.map((id) => WEB_JID_PREFIX + sanitizeSid(id)),
            );
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end('{"ok":true}');
          } catch {
            ...
            if (content && typeof content === 'string') {
              ...
              if (cwdCmdMatch) {
                ...
                broadcastToSession(
                  sessionId,
                  'cwd',
                  JSON.stringify(sessionCwds.get(sessionId) ?? ''),
                );
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end('{"ok":true}');
                return;
